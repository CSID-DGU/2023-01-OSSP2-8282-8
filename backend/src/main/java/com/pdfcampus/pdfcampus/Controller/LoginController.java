...
public class LoginController {
    ...
    @PostMapping("/login")
    public ResponseEntity<Map<String, Object>> login(@RequestBody SignupDto signupDTO) {
        // 로그인 정보를 검증하고 결과를 반환하는 로직
        boolean isLoginValid = loginService.checkLogin(signupDTO.getUserId(), signupDTO.getPassword());
        if (isLoginValid) {
            // 로그인 정보가 올바르면 accessToken과 refreshToken을 발급받아 반환
            Map<String, String> tokens = loginService.issueTokens(signupDTO.getUserId());
            ...
            responseData.put("userId", 1); // 실제 사용자 ID로 변경해야 함
            responseData.put("isSubscribed", true); // 실제 구독 여부로 변경해야 함
